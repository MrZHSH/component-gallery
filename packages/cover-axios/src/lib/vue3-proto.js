export function proto_3(Vue, _lodash, $axios) {
  /**
   * [$GET 全局的 GET 方法]
   *
   * @method $GET
   ...
   */
  (Vue.config.globalProperties.$GET = async (url, payload) => {
   ...
   */
  (Vue.config.globalProperties.$POST = async (url, payload) => {
    // eslint-disable-next-line no-useless-catch
    try {
   ...
   */
  (Vue.config.globalProperties.$PATCH = async (url, payload) => {
    // eslint-disable-next-line no-useless-catch
    try {
      if (_lodash.isArray(payload) || _lodash.isObject(payload)) {
   ...
   */
  (Vue.config.globalProperties.$DELETE = async (url, payload) => {
    // eslint-disable-next-line no-useless-catch
    try {
      let response = null
      if (_lodash.size(payload)) {
   ...
   */
  (Vue.config.globalProperties.$PUT = async (url, payload) => {
    // eslint-disable-next-line no-useless-catch
    try {
      if (_lodash.isArray(payload) || _lodash.isObject(payload)) {
